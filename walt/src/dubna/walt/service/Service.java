...
//        debugPrintln(".");
//        debugPrint(" ====> START SERVICE " + this.getClass()  + "; dbUtil=" + dbUtil );
//        if(dbUtil != null)
...
//        debugPrintln(";  c=" + cfgTuner.getParameter("c") + "; ");
...
//                );
//            }
            dbUtil = new DBUtil( rm.getString("DB" + db, true, "")
                    , cfgTuner.getParameter("connString" + db)
...
//			   + cfgTuner.getParameter("connString" + db)
        ...
        System.out.println(" OK! " + dbUtil.myName);
    }
    ...
    private void execGetDataCmd(String s) throws Exception {
        ...
        if (s.length() > 5) {
            ...
            if (s.indexOf("read file:") == 0) {
                ...
                int i = cmd.indexOf(":=");
                if (i > 0) {
                    paramName = cmd.substring(0, i - 1).trim();
                    cmd = cmd.substring(i + 1).trim();
                }
                cfgTuner.addParameter(paramName, executeCommand(cmd));
            } else if (s != null) {
//            } else {
                getPreData(s);
            }
...
//            dbUtil.release();
    ...
    public void readDataFile(String fileName, int maxLength) // throws Exception
    {
        ...
        if (allowedFiles.length() < 2
            ...
            System.out.println("<=== Reading file: " + startPath + fileName);
            try {
...
//      cfgTuner.readFile( startPath + fileName);
                ...
                cfgTuner.addParameter("contents1", cont);
            } catch (Exception e) {
                cfgTuner.addParameter("contents1", "NEW FILE");
            }
        } else {
    ...
    public void logSubserviceStarted(Tuner serviceTuner, String thisCfg) {
           ...
           )
        {
            IOUtil.writeLog(1, "<br><span class='ss_start pt' "
                    ...
                    + thisCfg + " => " + ss_c + "</b> (request:"
                    + rm.getString("queryLabel") + ")</span><div id='" + (ss_c + tm) + "' class='ss'>", rmg);
